...
import { ArrowLeft, Sparkle, MapPin, Clock, Star, Bookmark, Wifi, Plug, Users, Info, Sparkles } from 'lucide-react';
...
export default function SpotDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      ...
      // Fetch spot
      try {
        const response = await fetch(`http://127.0.0.1:8000/spots/${id}`);
        if (response.ok) {
  ...
  const handleBookmark = async () => {
    ...
    try {
      if (isSaved) {
        await fetch(`http://127.0.0.1:8000/users/${identifier}/saved/${id}`, { method: 'DELETE' });
        setIsSaved(false);
      } else {
        await fetch(`http://127.0.0.1:8000/users/${identifier}/saved/${id}`, { method: 'POST' });
        setIsSaved(true);
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#F4F7FB] pb-10">
      ...
      <div className="relative h-[40vh] md:h-[50vh] w-full">
        ...
        <div className="absolute bottom-8 left-6 right-6 text-white">
          ...
          <h1 className="text-3xl md:text-5xl font-black tracking-tight mb-2">{spot.name}</h1>
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 border border-white">
              <div className="flex items-center gap-4 mb-8 pb-8 border-b border-slate-100">
                <div className="h-14 w-14 bg-[#e6f2e7] rounded-2xl flex items-center justify-center text-[#0f3915]">
                  <Clock size={28} strokeWidth={2.5} />
                </div>
                <div>
                  <h3 className="text-slate-500 text-sm font-bold uppercase tracking-wider">Current Status</h3>
                  <p className="text-xl font-extrabold text-slate-900">{spot.status}</p>
                </div>
                <div className="ml-auto">
                   <span className={`px-4 py-2 rounded-full text-xs font-black uppercase ${
                     spot.occupancy === 'Low' ? 'bg-emerald-100 text-emerald-700' :
              ...
              <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100">
                <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <Sparkles size={16} className="text-[#0f3915]" /> The Vibe
                </h3>
            ...
            {/* Amenities Grid */}
            <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 border border-white">
              <h3 className="text-lg font-black text-slate-900 mb-6">Amenities & Features</h3>
          ...
          <div className="flex flex-col gap-6">
            <div className="bg-white rounded-[2.5rem] p-6 shadow-xl shadow-slate-200/50 border border-white sticky top-24">
              ...
              <button 
                ...
                className="w-full bg-slate-900 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2 hover:bg-slate-800 transition-all active:scale-[0.98] shadow-lg shadow-slate-900/20 border border-[#e6f2e7]"
              >
                Get Directions
              </button>
            </div>
